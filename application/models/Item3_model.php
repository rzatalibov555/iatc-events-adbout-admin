...
class Item3_model extends CI_Model{
    ...
    {
        return $this->db->order_by('t_id','DESC')->where($where)->get($this->tableName)->result();
    }

    public function get_single($where = array())
    {
        return $this->db
            ->join('item_category3', 'item_category3.i_c3_id = items3.t_category',"left")
            ->join('item_status', 'item_status.i_s_id = items3.t_status',"left")
